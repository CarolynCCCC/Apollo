...
const messageHandlers: Record<string, (message: any) => void> = {
  [WS_MESSAGE_TYPES.PLAYER_JOINED]: (message) => {
    console.log('Handling PLAYER_JOINED:', message);
    if (message.room) {
  ...
  [WS_MESSAGE_TYPES.PLAYER_READY]: (message) => {
    console.log('Handling PLAYER_READY:', message);
    if (message.room) {
      const roomStore = useRoomStore();
      roomStore.currentRoom = message.room;
    }
  ...
  [WS_MESSAGE_TYPES.GAME_STARTED]: (message) => {
    console.log('Handling GAME_STARTED:', message);
    const roomStore = useRoomStore();
  ...
  [WS_MESSAGE_TYPES.TEAM_PROPOSED]: (message) => {
    console.log('Handling TEAM_PROPOSED:', message);
  ...
  [WS_MESSAGE_TYPES.REQUEST_TEAM_VOTE]: (message) => {
    console.log('Handling REQUEST_TEAM_VOTE:', message);
  },
  [WS_MESSAGE_TYPES.TEAM_VOTE_CAST]: (message) => {
    console.log('Handling TEAM_VOTE_CAST:', message);
  ...
  [WS_MESSAGE_TYPES.TEAM_VOTE_RESULT]: (message) => {
    console.log('Handling TEAM_VOTE_RESULT:', message);
  ...
  [WS_MESSAGE_TYPES.REQUEST_QUEST_VOTE]: (message) => {
    console.log('Handling REQUEST_QUEST_VOTE:', message);
  ...
  [WS_MESSAGE_TYPES.NEXT_ROUND_STARTED]: (message) => {
    console.log('Handling NEXT_ROUND_STARTED:', message);
    ...
    window.dispatchEvent(new CustomEvent('ws:nextRound', {
      detail: {
  ...
  [WS_MESSAGE_TYPES.GAME_ENDED]: (message) => {
    ...
    window.dispatchEvent(new CustomEvent('ws:gameEnded', {
      detail: message
    }));
  },
...
export function useWebSocket() {
  function connect(roomId: string, playerId: number): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      if (wsControls && wsControls.status.value === 'OPEN') {
